...
class Tract {
  ...
  process(inputSamples, parameterSamples, sampleIndex, bufferLength, seconds) {
    ...
    this.lastNoseSample = this._processNose(parameterSamples, bufferInterpolation, updateAmplitudes);
    outputSample += this.lastNoseSample;
  ...
  _updateConstrictions(constrictions) {
    ...
    if (update) {
      ...
      for (let index = -1; index < constrictions.length; index++) {
        ...
        const nasalOverlap = 0.08;
        if (constriction.index > this.nose.start && constriction.diameter < nasalOverlap) {
            this.velum.target = Math.max(this.velum.target, Math.min(0.4, 0.4 * ((nasalOverlap - constriction.diameter) / (nasalOverlap + this.nose.offset))));
        }
...
class Processor {
  ...
  process(inputSamples, parameterSamples, sampleIndex, bufferLength, seconds) {
    ...
    var noseOutput = this.tract.lastNoseSample || 0;
    ...
    noseOutput += this.tract.lastNoseSample || 0;
    ...
    this.lastNoseSample = noseOutput;
    return outputSample;
  }
...
class PinkTromboneWorkletProcessor extends AudioWorkletProcessor {
  ...
  process(inputs, outputs, parameters) {
    ...
    for (let sampleIndex = 0; sampleIndex < bufferLength; sampleIndex++) {
      ...
      ch0[sampleIndex] = outputSample;
      if (ch1) ch1[sampleIndex] = this.processor.lastNoseSample || 0;
    }

    this.processor.update(currentTime + bufferLength / sampleRate, constrictions);
